...
""" 
- This function creates a random tour using the shuffle method from random module, for use in the initialise function
...
"""
- This function initialises the positions and velocities to be used by the PSO algorithm
- It also assigns the first case of the best tour found from these random positions
"""
def initialise(N):
    ...
    # Initialisation + Finding best tour yet
    track_best = 0
    track_best_length = -1
    for i in range(0,N):
        temp = random_tour()
        pt[i] = temp
        pbt[i] = temp
        pbt_eval[i] = evaluate_position(temp)
        if (pbt_eval[i] < track_best_length) or (track_best_length == -1):
            track_best = pbt[i]
            track_best_length = pbt_eval[i]
        vt[i] = compute_vel(temp, random_tour())
    
    # Initialising the best tour so far
